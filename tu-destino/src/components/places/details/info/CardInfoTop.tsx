...
export default function CardInfoTop() {
  return (
      ...
      <div className=" max-w-[1300px] w-full px-3 sm:px-14">
        <section className="border-b border-[#DDDDDD] pb-10 grid grid-cols-2 sm:grid-cols-6 md:grid-rows-1">
          ...
          <Card
            ...
            title="Ubicación"
            text="Cl. 50, La Candelaria, Medellín, La Candelaria, Medellin, Antioquia"
          />
          <Card Icon={Schedule} title="Horario" />
          <Card Icon={AttachMoney} title="Precio" />
          <Card Icon={LocalPhoneRounded} title="Teléfono" />
          <Card Icon={Web} title="Sitio web" />
        </section>
      </div>
    </div>
  );
  ...
  return (
    <div className="col-span-1 h-24 lg:h-28 border-r pb-1 border-[#DDDDDD] flex justify-center">
      <div className=" h-full w-4/5 max-w-[125px] flex justify-between flex-col">
        <div className="">
          <h3 className="text-[13px] text-[#3c3c3c] md:text-base lg:text-lg">{title}</h3>
          <p className={`max-h-11 text-black ${poppins.className} overflow-scroll`}>{text}</p>
        </div>
        <div className="">
          {Icon && (
            <Icon
